...
const modalOverlayStyle = { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 0.7)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '20px' } // Add padding para mobile
...
const filterButtonStyle = (active, color) => ({ padding: '8px 16px', background: active ? color : 'transparent', border: `1px solid ${color}`, color: active ? '#fff' : color, borderRadius: '20px', cursor: 'pointer', fontWeight: 'bold', fontSize: '12px', transition: '0.2s', flex: '1 1 auto', textAlign: 'center' }) // Flex para botões se ajustarem
...
export default function Dashboard() {
  ...
  async function handleSendReport() {
    if (!confirm("Deseja enviar um relatório dos documentos vencidos e próximos para seu e-mail?")) return;
    ...
    try {
      ...
      if (criticalDocs.length === 0) {
        alert("Você não tem documentos vencidos ou próximos do vencimento para relatar.")
        setSendingEmail(false); return
      }

      let messageText = "⚠️ ATENÇÃO AOS SEGUINTES DOCUMENTOS:\n\n"
      criticalDocs.forEach(doc => {
        const days = differenceInDays(parseISO(doc.expiry_date), new Date())
        const status = days < 0 ? "[VENCIDO]" : "[ALERTA]"
        const dataFormatada = format(parseISO(doc.expiry_date), 'dd/MM/yyyy')
        messageText += `${status} ${doc.title} (${doc.category}) - Vence: ${dataFormatada}\n`
      })
      messageText += `\nTotal de itens críticos: ${criticalDocs.length}`

      await emailjs.send(SERVICE_ID, TEMPLATE_ID, {
          to_name: "Gestor", message: messageText, to_email: userEmail, reply_to: userEmail
        }, PUBLIC_KEY)
  ...
  async function handleUpdate(e) {
    e.preventDefault()
    try {
      const { error } = await supabase.from('documents').update({ title: editForm.title, expiry_date: editForm.date, category: editForm.category }).eq('id', editingDoc.id)
      if (error) throw error
      alert("Documento atualizado!"); setEditingDoc(null); fetchDocuments()
    } catch (error) { alert("Erro ao atualizar: " + error.message) }
  }

  async function handleUpload(e) {
    e.preventDefault()
    if (!newFile || !newTitle || !newDate || !newCategory) return alert("Preencha tudo!")
    setUploading(true)
    try {
      const { data: { user } } = await supabase.auth.getUser()
      if (!user) throw new Error("Faça login novamente.")
      const fileExt = newFile.name.split('.').pop(); const fileName = `${Date.now()}.${fileExt}`
      const { error: uploadError } = await supabase.storage.from('file-docs').upload(fileName, newFile)
      if (uploadError) throw uploadError
      const { data: { publicUrl } } = supabase.storage.from('file-docs').getPublicUrl(fileName)
      const { error: dbError } = await supabase.from('documents').insert([{ title: newTitle, expiry_date: newDate, category: newCategory, file_url: publicUrl, user_id: user.id }])
      if (dbError) throw dbError
      alert("Salvo com sucesso!"); setNewTitle(''); setNewDate(''); setNewCategory(''); setNewFile(null); document.getElementById('fileInput').value = ""; fetchDocuments()
    } catch (error) { alert("Erro: " + error.message) } finally { setUploading(false) }
  }

  async function handleDelete(id) {
    if(!confirm("Excluir definitivamente?")) return;
    try { const { error } = await supabase.from('documents').delete().match({ id }); if (error) throw error; fetchDocuments() } catch (error) { alert("Erro ao deletar: " + error.message) }
  }
  ...
  const filteredDocs = docs.filter(doc => {
    ...
    const statusColor = getStatusColor(doc.expiry_date)
    let statusType = 'ok'; if (statusColor === '#ff4d4d') statusType = 'vencidos'; if (statusColor === '#ffcc00') statusType = 'alerta'
    const matchesStatus = statusFilter === 'todos' || statusType === statusFilter
    ...
    <div style={containerStyle}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px' }}>
        ...
        <div style={{display: 'flex', gap: '10px', flexWrap: 'wrap'}}>
          <button onClick={handleSendReport} disabled={sendingEmail} style={{background: '#ffcc00', border: 'none', color: '#000', padding: '8px 16px', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold'}}>
            {sendingEmail ? 'Enviando...' : '📢 Enviar Relatório'}
          </button>
          <button onClick={handleLogout} style={{background: 'transparent', border: '1px solid #555', color: '#aaa', padding: '8px 16px', borderRadius: '6px', cursor: 'pointer'}}>Sair</button>
        </div>
      </header>

      {/* Upload - MOBILE FRIENDLY: Usei FlexWrap e minWidth */}
      <section style={cardStyle}>
        ...
        <form onSubmit={handleUpload} style={{ display: 'grid', gap: '15px' }}>
          ...
          {/* Container flexível para Data e Categoria */}
      ...
      {/* Filtros - MOBILE FRIENDLY: FlexWrap na busca */}
      ...
      {/* Lista - MOBILE FRIENDLY: FlexWrap nos cards */}
      <section>
          ...
          <div style={{ display: 'grid', gap: '15px' }}>
            {filteredDocs.map(doc => (
              <div key={doc.id} style={{ 
                ...
                alignItems: 'center', 
                flexWrap: 'wrap', // <--- O SEGREDO DO MOBILE
                gap: '15px',      // <--- ESPAÇAMENTO QUANDO QUEBRAR LINHA
                borderLeft: `6px solid ${getStatusColor(doc.expiry_date)}`, 
                boxShadow: '0 2px 5px rgba(0,0,0,0.1)' 
              }}>
                <div style={{flex: '1 1 200px'}}> {/* Texto ocupa espaço disponível */}
